"""`scitex-db skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-db/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-db/` directory."""
    import scitex_db

    pkg_dir = Path(scitex_db.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-db/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-db.

    \b
    Examples:
      $ scitex-db skills list
      $ scitex-db skills get 01_installation
      $ scitex-db skills install                  # → ~/.scitex/dev/skills/scitex-db/
      $ scitex-db skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-db skills get 01_installation
      $ scitex-db skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-db/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-db/` dir to
    ~/.scitex/dev/skills/scitex-db/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-db skills install
      $ scitex-db skills install --claude-symlink
      $ scitex-db skills install --no-link --dest /tmp/scitex-db-skills
    """
